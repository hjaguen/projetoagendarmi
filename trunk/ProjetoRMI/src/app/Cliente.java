...
public class Cliente {
	 ...
	 */
	public void fazerConvite(IServidor s, Agenda a) throws RemoteException,
		...
		while (nome != "" || c.size() == 0) {
			...
			System.out
					.println("Digite o Contato que você deseja convidar (em branco para sair): ");
		...
		try {
			...
			System.out
					.println("Digite a data do Evento: (Formato: dd/MM/yyyy HH:mm:ss) ");
			String dataString = entrada.nextLine();
			Date data = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
					.parse(dataString);
			ev.setData(data);
			...
			for (int i = 0; i <= c.size(); i++) {
				...
				if (true) {
					...
					// Se o usuário aceitar o convite, adiciona a uma lista onde
					// estarão os contatos que aceitaram o convite
					if (true) {
						co.add(cliente);
						contador++;
					} else
						System.out.println("Contato: " + cliente.getNome()
								+ " rejeitou seu convite");
				} else {
					System.out.println("Contato " + cliente.getNome()
							+ " não tem uma Agenda!");
				}
			...
			if (contador == 0) {
				System.out
						.println("Nenhum evento foi adicionado, os contatos recusaram o convite ou já estavam ocupados");
			} else
				// Se alguém aceitou o convite, adiciona o usuário que fez os
				// convites também na lista de contatos
			...
			a.addEventos(ev);
			System.out
					.println("Contatos foram adicionados com sucesso ao evento!");
